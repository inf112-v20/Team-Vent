...
- Come up with good solutions to handle the Corona situation
- User stories
...
3. As a player I want my robot to capture flags, so that I capture all the flags and win.
    ...
    - Tasks:
        - Update StateInfo/RobotState to represent the saveLocation and the number of captured flags
        - Refactor Robot so that it has a RobotState object to avoid duplication and protect robot from unwanted changes
        - Make a list of flagSteps, and make sure they are calculated and scheduled at the right time
